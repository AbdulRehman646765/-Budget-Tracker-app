...
export const MonthlyComparison: React.FC<MonthlyComparisonProps> = ({
  ...
  return (
    <section className="glass-card comparison-section tool-card" id="comparisonSection">
      <div className="section-title-row">
        <h2><i className="fa-solid fa-chart-bar"></i> Monthly Comparison</h2>
        <button className="icon-btn" onClick={onClear} title="Clear Monthly Data">
          <i className="fa-solid fa-trash-can"></i>
        </button>
      </div>
      <div className="comparison-chart" id="comparisonChart">
        {monthlyHistory.length === 0 ? (
          <p className="comparison-empty" id="comparisonEmpty">Save budgets to see monthly comparison.</p>
        ) : (
            ...
            return (
              <div className="comp-bar-wrapper" key={idx}>
                <div className="comp-bar-header">
                  <span>{m.month}</span>
                  <span>{currency} {m.expense.toLocaleString()}</span>
                </div>
                <div className="comp-bar-track">
                  <div
                    className="comp-bar-fill"
                    style={{ width: `${percent}%`, background: "linear-gradient(90deg, #6366f1, #38bdf8)" }}
                  >
                    {percent > 20 ? percent + "%" : ""}
                  </div>
                </div>
